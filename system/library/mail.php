	...
	public function send() {
		...
		if ($this->protocol == 'mail') {
			...
			if (!$handle) {
				...
				$code = substr($this->get_data($handle),0,3); // проверяем, не возвратил ли сервер ошибку.
				if($code != 250) {throw new \Exception("ошибка приветсвия EHLO"); fclose($handle); exit;}

				if (!empty($this->smtp_username)  && !empty($this->smtp_password)) {
					fputs($handle,"AUTH LOGIN\r\n"); // начинаем процедуру авторизации.
					$code = substr($this->get_data($handle),0,3);
					if($code != 334) {throw new \Exception("сервер не разрешил начать авторизацию"); fclose($handle); exit;}
					...
					$code = substr($this->get_data($handle),0,3);
					if($code != 334) {throw new \Exception("ошибка доступа к такому юзеру"); fclose($handle); exit;}

					fputs($handle,base64_encode($this->smtp_password)."\r\n");       // отправляем серверу пароль.
					$code = substr($this->get_data($handle),0,3);                 
					if($code != 235) {throw new \Exception("неправильный пароль"); fclose($handle); exit;}
				}
				
				if ($this->verp) {
					fputs($handle,"MAIL FROM:<".$this->smtp_username.">XVERP". PHP_EOL); // отправляем серверу значение MAIL FROM.
					$code = substr($this->get_data($handle),0,3);
					if($code != 250) {throw new \Exception("сервер отказал в команде MAIL FROM"); fclose($handle); exit;}
				} else {
				...
				if (!is_array($this->to)) {
					...
					$code = substr($this->get_data($handle),0,3);
					if($code != 250 AND $code != 251) {throw new \Exception("Сервер не принял команду RCPT TO"); fclose($handle); exit;}
				} else {
				...
				$code = substr($this->get_data($handle),0,3);
				if($code != 354) {throw new \Exception("сервер не принял DATA"); fclose($handle); exit;}
				...
				$code = substr($this->get_data($handle),0,3);
				if($code != 250) {throw new \Exception("ошибка отправки письма"); fclose($handle); exit;}

				fputs($handle,"QUIT\r\n");   // завершаем отправку командой QUIT.
				fclose($handle); // закрываем соединение.
			}
		}
	}
}
