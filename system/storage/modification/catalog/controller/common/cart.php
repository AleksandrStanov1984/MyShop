...
class ControllerCommonCart extends Controller {
	public function index() {
		...
		foreach ($this->cart->getProducts() as $product) {
			...
			if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
			   ...
			   if(!empty($wishlist_pd)) {
                ...
                if ($price) {
                  $price = '<b>'.rtrim(preg_replace("/[^0-9\.]/", "", $price), '.') . '</b><span class=\'currency-value\'>'. $this->currency->getSymbolRight($currency_code).'</span>';
              }
              if ($total) {
                  $total = '<b>'.rtrim(preg_replace("/[^0-9\.]/", "", $total), '.') . '</b><span class=\'currency-value\'>'. $this->currency->getSymbolRight($currency_code).'</span>';
              }

            
			$data['products'][] = array(
                'base_price' => (isset($product['base_price'])&&$product['base_price'])?$this->currency->format($this->tax->calculate($product['base_price'], $product['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']):false,
		...
		foreach ($totals as $total) {
                ...
                $total_text = '<b>'.rtrim(preg_replace("/[^0-9\.]/", "", $total_text), '.') . '</b><span class=\'currency-value\'>'. $this->currency->getSymbolRight($currency_code).'</span>';
