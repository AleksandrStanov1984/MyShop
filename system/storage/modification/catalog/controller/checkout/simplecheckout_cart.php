...
class ControllerCheckoutSimpleCheckoutCart extends SimpleController {
    ...
    public function index() {
        ...
        $this->_templateData['column_name']          = $this->language->get('column_name');
         $this->_templateData['column_size']          = $this->language->get('column_size');
        ...
        $this->_templateData['unit_h']               = $this->language->get('unit_h');
        $this->_templateData['unit_w']               = $this->language->get('unit_w');
        $this->_templateData['unit_l']               = $this->language->get('unit_l');
         $this->_templateData['unit']                = $this->language->get('unit');
          $this->_templateData['placeholder_cupon']               = $this->language->get('placeholder_cupon');
      
        $this->_templateData['text_until_cancelled'] = $this->language->get('text_until_cancelled');
        $this->_templateData['text_title_simplecart'] = $this->language->get('text_title_simplecart');
        ...
        foreach ($products as $product) {
            ...
            if ($version >= 200) {
                ...
                $this->_templateData['products'][] = array(
                'base_price'      => (isset($product['base_price'])&&$product['base_price'])?$this->simplecheckout->formatCurrency($this->tax->calculate($product['base_price'], $product['tax_class_id'], $this->config->get('config_tax'))):false,
        ...
        $this->_templateData['free_shipping'] = isset($this->session->data['free_shipping']) ? $this->session->data['free_shipping'] : '';
        ...
        if ($currentTheme == 'shoppica' || $currentTheme == 'shoppica2') {
            ...
            if (strpos($text_items, '{quantity}') !== false || strpos($text_items, '{total}') !== false) {
                ...
                $this->_templateData['cart_total'] = $this->cart->countProducts() ? $this->cart->countProducts() : 0;
            }
