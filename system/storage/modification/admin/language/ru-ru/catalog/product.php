<?php

$_['smallproductseditor_action'] = '- действия -';
$_['smallproductseditor_action_active'] = 'активировать';
$_['smallproductseditor_action_deactive'] = 'деактивировать';
$_['smallproductseditor_action_move'] = 'перенести в раздел';
$_['smallproductseditor_action_copy'] = 'добавить привязку к разделу';
$_['smallproductseditor_action_price'] = 'умножить цену на коэффициент';
$_['smallproductseditor_button'] = 'Применить';
$_['smallproductseditor_success'] = 'Успешно сохранено';
      

          $_['tab_hpm']        = '<b style="color:#4b0082;"><i class="fa fa-sitemap fa-1x fa-rotate-90"></i> HPM</b>';
      
// Heading
$_['heading_title']          	= 'Товары';

// Text
$_['text_success']          	= 'Список товаров обновлен!';
// OCFilter start
$_['entry_values']              = 'Добавьте значения для этой опции.';
$_['tab_ocfilter']              = 'Опции фильтра';
$_['ocfilter_select_category']  = 'Для начала, выберите категории для этого товара.';
// OCFilter end
$_['text_list']          		= 'Список товаров';
$_['text_add']          		= 'Добавление товара';
$_['text_edit']         	 	= 'Редактирование товара';
$_['text_plus']          		= '+';
$_['text_minus']       	 		= '-';
$_['text_default']        		= 'Основной магазин';
$_['text_option']		        = 'Опция';
$_['text_option_value']         = 'Значение опции';
$_['text_percent']          	= 'Процент';
$_['text_amount']         	 	= 'Фиксированная сумма';

// Column
$_['column_name']          		= 'Наименование товара';
$_['column_model']          	= 'Модель';
$_['column_image']          	= 'Изображение';
$_['column_category']         = 'Категория';
$_['column_price']          	= 'Цена на сайте';
$_['column_quantity']          	= 'Количество';
$_['column_status']          	= 'Статус';
$_['column_action']          	= 'Действие';

// Entry
$_['entry_main_category']       = 'Главная категория:';

$_['entry_category_filter'] = 'Категория';
$_['select_no_manufacturers'] = 'Без производителей';
$_['entry_attributes'] = 'Атрибут';
        
$_['entry_name']          		= 'Наименование:';
$_['entry_description']         = 'Описание:';
$_['entry_meta_title']          = 'HTML-тег Title';
$_['entry_meta_h1']             = 'HTML-тег H1';
$_['entry_meta_keyword']        = 'Мета-тег Keywords:';
$_['entry_meta_description']    = 'Мета-тег Description:';
$_['entry_keyword']          	= 'SEO URL:';
$_['entry_model']          		= 'Модель:';
$_['entry_sku']          		= 'Артикул:';
$_['entry_upc']          		= 'UPC:';
$_['entry_ean']          		= 'Vendor Prefix';
$_['entry_jan']          		= 'Vendor SKU:';
$_['entry_isbn']          		= 'Vendor ID:';
$_['entry_mpn']          		= 'MPN:';
$_['entry_video']            = 'Видео:';
$_['entry_video_title']            = 'Название:';
$_['entry_video_link']            = 'сылка на видео:';
$_['tab_video']            = 'Видео';
$_['button_video_add']     = 'Добавить видео';
$_['help_video']             = 'Пример: fmIiN5eHt90, NeinUp6KxMU';
$_['entry_location']          	= 'Расположение:';
$_['entry_shipping']          	= 'Необходима доставка:';
$_['entry_visible_carusel']    	= 'Показывать в карусели "Популярных товаров":';
$_['entry_manufacturer']        = 'Производитель:';
$_['entry_store']          		= 'Магазины:';
$_['entry_date_available']      = 'Дата поступления:';
$_['entry_quantity']          	= 'Количество:';
$_['entry_minimum']          	= 'Минимальное количество:';
$_['entry_stock_status']        = 'Отсутствие на складе:';
$_['entry_price']          		= 'Цена:';
$_['entry_tax_class']          	= 'Налог:';
$_['entry_points']          	= 'Баллы:';
$_['entry_option_points']       = 'Баллы:';
$_['entry_subtract']          	= 'Вычитать со склада:';
$_['entry_weight_class']        = 'Единица веса:';
$_['entry_weight']          	= 'Вес:';
$_['entry_dimension']          	= 'Размеры (В x Ш x Г):';
$_['entry_length_class']        = 'Единица длины';
$_['entry_length']          	= 'Единица длины:';
$_['entry_width']      		= 'Ширина';
$_['entry_height']          	= 'Высота';


                $_['entry_quantity_shelves']       	= 'Количество полок:';
								$_['entry_maximum_weight']         	= 'Максимальная нагрузка на полку(кг):';
								$_['entry_maximum_weight_all']     	= 'Максимальная нагрузка на стеллаж(кг):';
								$_['entry_material_shelves']        = 'Материал полки:';
								$_['entry_model_selection']         = 'Модель в подбор:';
								$_['entry_color_shelves']          	= 'Цвет:';
								$_['entry_metal_thickness']         = 'Толщина металла:';
								$_['entry_type']         						= 'Тип замка:';
								$_['entry_series']         					= 'Серия:';
								$_['entry_brand']         					= 'Бренд:';
								$_['entry_filter_status']          	= 'Использовать в подборе:';
            
$_['entry_image']          		= 'Изображение товара:';
$_['entry_price_base']          = 'Цена себестоимости:';
$_['entry_image_dod']          	= 'Главное изображение с водяным знаком';
$_['entry_additional_image'] 	= 'Дополнительные изображения';
$_['entry_customer_group']      = 'Группа покупателей:';
$_['entry_date_start']          = 'Дата начала:';
$_['entry_date_end']          	= 'Дата окончания:';
$_['entry_priority']          	= 'Приоритет:';
$_['entry_attribute']          	= 'Атрибут:';
$_['entry_attribute_group']     = 'Группа атрибутов:';
$_['entry_text']          		= 'Текст:';
$_['entry_option']          	= 'Опция:';
$_['entry_option_value']        = 'Значение:';
$_['entry_required']          	= 'Обязательно:';
$_['entry_status']          	= 'Статус:';
$_['entry_sort_order']          = 'Порядок сортировки:';
$_['entry_category']          	= 'Показывать в категориях:';
$_['entry_filter']          	= 'Фильтры:';
$_['entry_download']          	= 'Загрузки:';
$_['entry_related']          	= 'Сопутствующие товары:';
$_['entry_related_info']      = '<br /><b style="color:red">Модель</b><br /><b style="color:blue">Артикул</b><br /><b style="color:green">Размеры (ВхШхГ)</b>';
$_['entry_tag']          		= 'Теги товара:';
$_['entry_reward']          	= 'Бонусные баллы:';
$_['entry_layout']          	= 'Переопределить макет:';
$_['entry_recurring']          	= 'Профили с регулярными платежами';

$_['entry_diameter_of_pot'] = 'Диаметр';
$_['entry_depth_of_pot'] = 'Глубина';
$_['entry_pot_size'] = 'Диаметр и глубина горшка';
$_['entry_price_rozetka'] = 'Цена для розетки';
$_['entry_price_special_rozetka'] = 'Акционная цена для розетки';

// Help
$_['help_keyword']          	= 'Замените пробелы на тире. Должно быть уникальным на всю систему.';
$_['help_sku']          		= 'Складской номер';
$_['help_upc']          		= 'Универсальный код товара';
$_['help_ean']          		= 'В базе поле ean';
$_['help_jan']          		= 'В базе поле jan';
$_['help_isbn']          		= 'В базе поле isbn';
$_['help_mpn']          		= 'Номер партии изготовителя';
$_['help_manufacturer']         = '(Автодополнение)';
$_['help_minimum']          	= 'Минимальное количество для заказа этого товара';
$_['help_stock_status']         = 'Статус, показываемый, когда товара нет на складе';
$_['help_points']          		= 'Количество баллов для покупки товара. Поставьте 0, чтобы товар нельзя было приобрести за баллы.';
$_['help_category']          	= '(Автодополнение)';
$_['help_filter']          		= '(Автодополнение)';
$_['help_download']          	= '(Автодополнение)';
$_['help_related']          	= '(Автодополнение)';
$_['help_tag']          		= 'разделяются запятой';
$_['tab_privat']				= 'Приватбанк';
$_['credit_type']				= 'Вид кредитования:';
$_['partsCount']				= 'Максимум месяцев:';
$_['markup']					= 'Коэффициент наценки:';
$_['help_partsCount']			= 'Введите максимальное количество месяцев кредитования';
$_['help_markup']				= 'Если необходимо, установите наценку на стоимость товара, например 1.027, что составляет 2.7%';
$_['credit_type_ii']			= 'Мгновенная рассрочка:';
$_['credit_type_pp']			= 'Оплата частями:';

// Error
$_['error_warning']          	= 'Внимательно проверьте форму на ошибки!';$_['error_keyword']          	= 'Этот SEO keyword уже используется!';
$_['error_permission']          = 'У вас нет прав для изменения товаров!';
$_['error_name']          		= 'Наименование товара должно быть от 3 до 255 символов!';
$_['error_meta_title']          = 'Мета-тег Title должен быть от 3 до 255 символов!';
$_['error_model']          		= 'Модель товара должна быть от 3 до 64 символов!';
$_['error_keyword']             = 'ВНИМАНИЕ: SEO keyword уже используется!';


//Delivery rates
$_['tab_delivery']        = 'Тарифы доставки';
$_['entry_np_branch']     = 'НП отделение';
$_['entry_np_courier']    = 'НП  курьер';
$_['entry_np_sz_courier'] = 'НП SZ курьер';
//Delivery rates