...
class ModelDesignLayout extends Model {
	public function addLayout($data) {
        ...
        if (isset($data['layout_route'])) {
            foreach ($data['layout_route'] as $layout_route) {
                $this->db->query("INSERT INTO " . DB_PREFIX . "layout_route_type SET layout_type = '" . (int)$data['type'] . "', layout_id = '" . (int)$layout_id . "', store_id = '" . (int)$layout_route['store_id'] . "', route = '" . $this->db->escape($layout_route['route']) . "'");
            }
        }
        // >
	...
	public function editLayout($layout_id, $data) {
	    ...
	    // < sha_layouts_type
        $this->db->query("DELETE FROM " . DB_PREFIX . "layout_route_type WHERE layout_id = '" . (int)$layout_id . "'");
    ...
    // < sha_layouts_type
    public function getLayoutType($layout_id) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "layout_route_type WHERE layout_id = '" . (int)$layout_id . "'");

        return isset($query->row['layout_type']) ? $query->row['layout_type'] : 0;
    }
    // >
