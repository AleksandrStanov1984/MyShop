...
class ControllerCatalogManufacturer extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if ($this->config->get('module_catalog_quick_edit_status') && $this->config->get('module_catalog_quick_edit_catalog_manufacturers_status')) {
	...
	protected function getList() {
      ...
      foreach ($this->config->get('module_catalog_quick_edit_catalog_manufacturers') as $column => $attr) {
		...
		foreach ($results as $result) {
            if($result['image']) {
                $image = $result['image'];
                $image_th = $this->model_tool_image->resize($result['image'], 100, 100);
               } else {
                $image_th = $this->model_tool_image->resize('no_image.png', 100, 100);
                $image = '';
            }
    ...
    public function upload()
    {
        ...
        if (isset($this->request->get['id'])) {
            $this->load->model('tool/image');
            $json['thumb'] = $this->model_tool_image->resize($this->request->get['image'], 100, 100);
            $this->db->query("UPDATE " . DB_PREFIX . "manufacturer SET image = '" . $this->db->escape($this->request->get['image']) . "' WHERE manufacturer_id = '" . (int)$this->request->get['id'] . "'");
            $json['success'] = 1;
        }
