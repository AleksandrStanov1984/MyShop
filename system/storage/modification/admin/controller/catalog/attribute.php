...
class ControllerCatalogAttribute extends Controller {
    ...
    public function add()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            if ($this->config->get('module_catalog_quick_edit_status') && $this->config->get('module_catalog_quick_edit_catalog_attributes_status')) {
    ...
    public function edit()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $json = $this->model_catalog_attribute->editAttribute($this->request->get['attribute_id'], $this->request->post);
    ...
    public function delete_attribute_values()
    {

        $this->load->model('catalog/attribute');

        if (isset($this->request->get['attribute_id'])) {
            $this->model_catalog_attribute->deleteAttributeValues($this->request->get['attribute_id']);
        }
    }
    ...
    public function delete()
    {
        ...
        if (isset($this->request->post['selected']) && ($this->validateDelete() || (int)$this->request->post['group'] > 0 || isset($this->request->post['delete_product_value']))) {

            if ((int)$this->request->post['group'] > 0) {

                foreach ($this->request->post['selected'] as $i => $row) {
                    if ((int)$this->request->post['group'] == (int)$row) unset($this->request->post['selected'][$i]);
                }

                //$attr_id = array_shift($this->request->post['selected']);
                $attr_id = (int)$this->request->post['group'];
                $this->model_catalog_attribute->groupAttribute($attr_id, $this->request->post['selected']);

            }

            foreach ($this->request->post['selected'] as $attribute_id) {
                if (isset($this->request->post['delete_product_value'])) {
                    $this->model_catalog_attribute->deleteAttributeProductValues($attribute_id);
                }
    ...
    {
        $slice = (int)$this->request->get['slice'];
        $attribute_id = (int)$this->request->get['attribute_id'];

        $this->load->model('catalog/attribute');
        $this->model_catalog_attribute->update_attribute_values();
    ...
    protected function getForm()
    {
        ...
        $data['attribute_values'] = $this->model_catalog_product->getAllProductAttributes($this->request->get['attribute_id']);
    ...
    public function autocomplete_values()
    {
        $json = array();
        $this->load->model('catalog/attribute');
        ...
        if (isset($this->request->get['attribute_id'])) {
            ...
            //Получим есть ли закешированные атрибуты
            $total = $this->model_catalog_attribute->getAttributeValuesTotal($this->request->get['attribute_id']);

            //Если ничего нет - закешируем
            if ($total == 0) {
                $this->load->model('catalog/product');
                $results = $this->model_catalog_product->getAllProductAttributes($this->request->get['attribute_id']);
                $this->model_catalog_attribute->updateAttributeValues($this->request->get['attribute_id'], $results);
            }

            $results = $this->model_catalog_attribute->getAttributeValues($this->request->get['attribute_id'], $this->request->get['filter_name']);

            $values = array();
            foreach ($results as $result) {
                $values[$result['value_key']][$result['language_id']] = $result['text'];
            }

            foreach ($values as $key => $result) {
                $json[] = array(
                    'key' => $key,
                    'name' => strip_tags(html_entity_decode(implode(';', $result), ENT_QUOTES, 'UTF-8')),
                );
            }
        }
    ...
    public function removingNonProductSpecificAttribute()
    {
            $this->load->model('catalog/attribute');
            $data = $this->model_catalog_attribute->removingNonProductSpecificAttributes();
