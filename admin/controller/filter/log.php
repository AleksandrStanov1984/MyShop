<?php
class ControllerFilterLog extends Controller {
	private $error = array();

	public function index() {		
		$this->load->language('tool/log');
		
		$this->document->setTitle('Log копирования OC Filter');
		...
		$this->response->setOutput($this->load->view('filter/log', $data));
	}

	public function download() {
		$this->load->language('tool/log');

		$file = DIR_LOGS . 'copyFilter.txt';
		...
		if (file_exists($file) && filesize($file) > 0) {
			...
			$this->response->redirect($this->url->link('filter/log', 'token=' . $this->session->data['token'], true));
		}
	}
	...
	public function clear() {
		...
		if (!$this->user->hasPermission('modify', 'filter/log')) {
