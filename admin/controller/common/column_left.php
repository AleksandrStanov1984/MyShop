      ...
      if ($newsblog) {
        $data['menus'][] = array(
          ...
          'children' => $newsblog
			...
			if ($this->user->hasPermission('access', 'catalog/category_mgr')) { $this->load->language('catalog/category_mgr'); $catalog[] = array( 'name' => $this->language->get('category_mgr_heading_title'), 'href' => $this->url->link('catalog/category_mgr', 'token=' . $this->session->data['token'], true), 'children' => array() ); }
			...
			if ($this->user->hasPermission('access', 'catalog/attribute_color')) {
				$attribute[] = array(
					'name'     => 'Палитра цветов',
					'href'     => $this->url->link('catalog/attribute_color', 'token=' . $this->session->data['token'], true),
					'children' => array()	
				);
			}
			...
			if ($this->user->hasPermission('access', 'catalog/folder_search')) {		
				$catalog[] = array(
					'name'	   => 'Фонетический поиск',
					'href'     => $this->url->link('catalog/folder_search', 'token=' . $this->session->data['token'], true),
					'children' => array()
				);
			}			
			
			if ($this->user->hasPermission('access', 'catalog/tag')) {		
				$catalog[] = array(
					'name'	   => 'Теги поиска',
					'href'     => $this->url->link('catalog/tag', 'token=' . $this->session->data['token'], true),
					'children' => array()
				);
			}
			...
			//Реестр поставщиков
            $provider = array();
            if ($this->user->hasPermission('access', 'provider/provider_name')) {
                $provider[] = array(
                    'name'     => 'Поставщики',
                    'href'     => $this->url->link('provider/provider_name', 'token=' . $this->session->data['token'], true),
                    'children' => array()
                );
            }

            if ($this->user->hasPermission('access', 'provider/provider_ship')) {
                $provider[] = array(
                    'name'     => 'Методы Доставки',
                    'href'     => $this->url->link('provider/provider_ship', 'token=' . $this->session->data['token'], true),
                    'children' => array()
                );
            }

            if ($this->user->hasPermission('access', 'provider/provider_delivery')) {
                $provider[] = array(
                    'name'     => 'Тарифы Доставки',
                    'href'     => $this->url->link('provider/provider_delivery', 'token=' . $this->session->data['token'], true),
                    'children' => array()
                );
            }


            if ($this->user->hasPermission('access', 'provider/product')) {
                $provider[] = array(
                    'name'     => 'Продукты Поставщиков',
                    'href'     => $this->url->link('provider/product', 'token=' . $this->session->data['token'], true),
                    'children' => array()
                );
            }


            if ($this->user->hasPermission('access', 'provider/delivery')) {
                $provider[] = array(
                    'name'     => 'Тарифы доставки (OLD)',
                    'href'     => $this->url->link('provider/delivery', 'token=' . $this->session->data['token'], true),
                    'children' => array()
                );
            }
			...
			$import = array();
			if ($this->user->hasPermission('access', 'import/brain')) {		
				$import[] = array(
					'name'	   => 'API Brain',
					'href'     => $this->url->link('import/brain', 'token=' . $this->session->data['token'], true),
					'children' => array()
				);
			}


			if ($this->user->hasPermission('access', 'import/attribute')) {		
				$import[] = array(
					'name'	   => 'Атрибуты',
					'href'     => $this->url->link('import/attribute', 'token=' . $this->session->data['token'], true),
					'children' => array()
				);
			}			
			
			if ($this->user->hasPermission('access', 'import/url')) {		
				$import[] = array(
					'name'	   => 'Модуль URL',
					'href'     => $this->url->link('import/url', 'token=' . $this->session->data['token'], true),
					'children' => array()
				);
			}			
            ...
            //brain API new
            $brain_api = array();
            if($this->user->hasPermission('access', 'brain/log')) {
                $brain_api[] = array(
                    'name' => 'Log Обновления цен',
                    'href' => $this->url->link('brain/log', 'token=' . $this->session->data['token'], true),
                    'children' => array()
                );
            }

            if($this->user->hasPermission('access', 'brain/price')) {
                $brain_api[] = array(
                    'name' => 'Корректировка Цен',
                    'href' => $this->url->link('brain/price', 'token=' . $this->session->data['token'], true),
                    'children' => array()
                );
            }


            if ($this->user->hasPermission('access', 'brain/brain_result')) {
                $brain_api[] = array(
                    'name'     => 'Обновления Цен',
                    'href'     => $this->url->link('brain/brain_result', 'token=' . $this->session->data['token'], true),
                    'children' => array()
                );
            }
            ...
            if($this->user->hasPermission('access', 'filter/ocfilter')) {
                $filter[] = array(
                    'name' => 'Копирование OC Filter',
                    'href' => $this->url->link('filter/ocfilter', 'token=' . $this->session->data['token'], true),
                    'children' => array()
                );
            }
            if($this->user->hasPermission('access', 'filter/log')) {
                    $filter[] = array(
                        'name' => 'Log OC Filter',
                        'href' => $this->url->link('filter/log', 'token=' . $this->session->data['token'], true),
                        'children' => array()
                    );
            }
            ...
            if ($this->config->get('hpm_status')) { $this->load->language('extension/module/handy_product_manager'); if ($this->user->hasPermission('access', 'extension/module/handy_product_manager')) { $hpm[] = array( 'name' => $this->language->get('text_hpm_product'), 'href' => $this->url->link('extension/module/handy_product_manager/productList', 'token=' . $this->session->data['token'], true), 'children' => array() ); $hpm[] = array( 'name' => $this->language->get('text_hpm_mass_edit'), 'href' => $this->url->link('extension/module/handy_product_manager/massEdit', 'token=' . $this->session->data['token'], true), 'children' => array() ); $data['menus'][] = array( 'id' => 'handy-product-manager', 'icon' => 'fa-hand-o-right fa-rotate-270', 'name' => $this->language->get('text_hpm_menu'), 'href' => '', 'children' => $hpm ); } }
			...
			*/	
			...
			if ($this->user->hasPermission('access', 'extension/module/import_yml')) {
				$tool[] = array(
					'name'	   => $this->language->get('text_import_yml'),
			...
			//new
            if ($this->user->hasPermission('access', 'tool/yml_replace')) {
                $tool[] = array(
                    'name'	   => 'Структура YML',
                    'href'     => $this->url->link('tool/yml_replace', 'token=' . $this->session->data['token'], true),
                    'children' => array()
                );
            }
           //new

			if ($this->user->hasPermission('access', 'extension/module/broken_links')) {
				$tool[] = array(
					'name'	   => 'Broken Links (404)',
					'href'     => $this->url->link('extension/module/broken_links/view_log', 'token=' . $this->session->data['token'], true),
					'children' => array()		
				);
			}

			if ($this->user->hasPermission('access', 'extension/feed/fasset_filter')) {
				$tool[] = array(
					'name'	   => 'Facet Filter',
            ...
            if ($this->user->hasPermission('access', 'tool/import_yml')) {
                $tool[] = array(
                    'name'	   => $this->language->get('text_import_yml'),
            ...
            if ($this->user->hasPermission('access', 'tool/log_feed')) {
                $tool[] = array(
                    'name'	   => 'Лог Google Feed',
                    'href'     => $this->url->link('tool/log_feed', 'token=' . $this->session->data['token'], true),
                    'children' => array()
                );
            }
