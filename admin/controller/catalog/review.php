...
class ControllerCatalogReview extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if ($this->config->get('module_catalog_quick_edit_status') && $this->config->get('module_catalog_quick_edit_catalog_reviews_status')) {
...
$this->load->model('tool/image');
		...
		if (isset($this->request->post['image1'])) {
                        $data['image'] = $this->request->post['image1'];
                } elseif (!empty($review_info)) {
                        $data['image1'] = $review_info['image1'];
                } else {
                        $data['image1'] = '';
                }
		...
		if (isset($this->request->post['image1']) && is_file(DIR_IMAGE . $this->request->post['image1'])) {
                ...
                } elseif (!empty($review_info) && is_file(DIR_IMAGE . $review_info['image1'])) {
                        $data['thumb1'] = $this->model_tool_image->resize($review_info['image1'], 100, 100);
                } else {
                        $data['thumb1'] = $this->model_tool_image->resize('no_image.png', 100, 100);
                }

		if (isset($this->request->post['image2'])) {
                        $data['image'] = $this->request->post['image2'];
                } elseif (!empty($review_info)) {
                        $data['image2'] = $review_info['image2'];
                } else {
                        $data['image2'] = '';
                }
		...
		if (isset($this->request->post['image2']) && is_file(DIR_IMAGE . $this->request->post['image2'])) {
                ...
                } elseif (!empty($review_info) && is_file(DIR_IMAGE . $review_info['image2'])) {
                        $data['thumb2'] = $this->model_tool_image->resize($review_info['image2'], 100, 100);
                } else {
...
$data['placeholder'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		...
		if (isset($this->request->post['dest'])) {
			$data['dest'] = $this->request->post['dest'];
		} elseif (!empty($review_info)) {
			$data['dest'] = $review_info['dest'];
		} else {
			$data['dest'] = '';
		}
