...
class ControllerExtensionFeedGoogleBase extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if (isset($this->request->post['google_base_categories'])) {
				$this->request->post['google_base_categories'] = implode(',', $this->request->post['google_base_categories']);
			}
			if (isset($this->request->post['google_base_categories_sz'])) {
				$this->request->post['google_base_categories_sz'] = implode(',', $this->request->post['google_base_categories_sz']);
			}
		...
		if (isset($this->request->post['google_base_categories'])) {
			$data['google_base_categories'] = $this->request->post['google_base_categories'];
		} elseif ($this->config->get('rozetka_categories') != '') {
			$data['google_base_categories'] = explode(',', $this->config->get('google_base_categories'));
		} else {
			$data['google_base_categories'] = array();
		}

		if (isset($this->request->post['google_base_categories_sz'])) {
			$data['google_base_categories_sz'] = $this->request->post['google_base_categories_sz'];
		} elseif ($this->config->get('rozetka_categories') != '') {
			$data['google_base_categories_sz'] = explode(',', $this->config->get('google_base_categories_sz'));
		} else {
			$data['google_base_categories_sz'] = array();
		}
	...
	public function category() {
		...
		if (isset($this->request->get['sz'])) {
			$results = $this->model_extension_feed_google_base->getCategoriesSZ(($page - 1) * 10, 10);
		} else {
		...
		if (isset($this->request->get['sz'])) {
			$category_total = $this->model_extension_feed_google_base->getTotalCategoriesSZ();
		} else {
		...
		if (isset($this->request->get['sz'])) {
			$pagination->url = $this->url->link('extension/feed/google_base/category', 'sz=1&token=' . $this->session->data['token'] . '&page={page}', true);
		} else {
		...
		if (isset($this->request->get['sz'])) {
			$this->response->setOutput($this->load->view('extension/feed/google_base_category_sz', $data));
		} else {
	...
	public function categorysz() {
		...
		$pagination->url = $this->url->link('extension/feed/google_base/categorysz', 'token=' . $this->session->data['token'] . '&page={page}', true);
	...
	public function addCategory() {
		...
		if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if (isset($this->request->post['sz'])) {
				$this->model_extension_feed_google_base->addCategorySZ($this->request->post);
			} else {
	...
	public function removeCategory() {
		...
		if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if (isset($this->request->get['sz'])) {
				$this->model_extension_feed_google_base->deleteCategorySZ($this->request->post['category_id']);
			} else {
