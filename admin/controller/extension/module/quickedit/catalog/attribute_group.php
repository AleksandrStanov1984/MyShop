<?php
class ControllerExtensionModuleQuickeditCatalogAttributeGroup extends Controller {
	...
	public function index() {
		...
		$this->load->model('extension/module/quickedit/catalog/attribute_group');

		$this->load->language('catalog/attribute_group');
		$this->load->language('extension/module/quickedit/catalog/general');
		$this->load->language('extension/module/quickedit/catalog/attribute_group');
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			...
			foreach($this->config->get('module_catalog_quick_edit_catalog_attribute_groups') as $column => $attr) {
			...
			if ((int)$this->config->get('module_catalog_quick_edit_override_menu_entry')) {
				...
				$this->response->redirect($this->url->link('extension/module/catalog_quick_edit/catalog__attribute_group__', 'user_token=' . $this->session->data['user_token'] . $url, true));
			}
		}
	...
	protected function getList() {
		...
		if (isset($this->request->get['sort'])) {
			...
			$sort = $this->config->get('module_catalog_quick_edit_catalog_attribute_groups_default_sort');
		}

		if (isset($this->request->get['order'])) {
			$order = $this->request->get['order'];
		} else {
			$order = $this->config->get('module_catalog_quick_edit_catalog_attribute_groups_default_order');
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href'      => (int)$this->config->get('module_catalog_quick_edit_override_menu_entry') ? $this->url->link('catalog/attribute_group', 'user_token=' . $this->session->data['user_token'] . $url . '&aqer=1', true) : $this->url->link('extension/module/catalog_quick_edit/catalog__attribute_group__', 'user_token=' . $this->session->data['user_token'] . $url, true),
			'active'    => true
		);
		...
		$columns = $this->config->get('module_catalog_quick_edit_catalog_attribute_groups');
		...
		$results = $this->model_extension_module_quickedit_catalog_attribute_group->getAttributeGroups($filter_data);

		$attribute_group_total = $this->model_extension_module_quickedit_catalog_attribute_group->getTotalAttributeGroups();
		...
		$data['update_url'] = html_entity_decode($this->url->link('extension/module/catalog_quick_edit/catalog__attribute_group__quick_update', 'user_token=' . $this->session->data['user_token'], true));
		$data['load_popup_url'] = html_entity_decode($this->url->link('extension/module/catalog_quick_edit/catalog__attribute_group__load_popup', 'user_token=' . $this->session->data['user_token'], true));
		...
		foreach ($this->config->get('module_catalog_quick_edit_catalog_attribute_groups') as $column => $attr) {
			if ((int)$this->config->get('module_catalog_quick_edit_override_menu_entry')) {
				...
				$data['sorts'][$column] = $this->url->link('extension/module/catalog_quick_edit/catalog__attribute_group__', 'user_token=' . $this->session->data['user_token'] . '&sort=' . $attr['sort'] . $url, true);
			}
		}
		...
		if ((int)$this->config->get('module_catalog_quick_edit_override_menu_entry')) {
			...
			$pagination->url = $this->url->link('extension/module/catalog_quick_edit/catalog__attribute_group__', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
		}
		...
		$template = 'extension/module/quickedit/catalog/attribute_group_list';
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			foreach ((array)$id as $_id) {
				if ($this->model_extension_module_quickedit_catalog_attribute_group->quickEditAttributeGroup($_id, $column, $value, $lang_id, $this->request->post)) {
					$results['done'][] = $_id;
				} else {
					$results['failed'][] = $_id;
				}
	...
	private function validatePermissions() {
		if (!$this->user->hasPermission('modify', 'catalog/attribute_group') || !$this->user->hasPermission('modify', 'extension/module/catalog_quick_edit')) {
