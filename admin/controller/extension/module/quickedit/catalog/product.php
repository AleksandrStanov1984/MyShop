<?php
class ControllerExtensionModuleQuickeditCatalogProduct extends Controller {
	...
	public function __construct($registry) {
		...
		if (!$this->config->get('module_catalog_quick_edit_installed') || !$this->config->get('module_catalog_quick_edit_status')) {
	...
	public function index() {
		...
		$this->load->model('extension/module/quickedit/catalog/product');

		$this->load->language('catalog/product');
		$this->load->language('extension/module/quickedit/catalog/general');
		$this->load->language('extension/module/quickedit/catalog/product');
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			...
			if ((int)$this->config->get('module_catalog_quick_edit_override_menu_entry')) {
				$this->response->redirect($this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . $url . '&aqer=1', true));
			} else {
				$this->response->redirect($this->url->link('extension/module/catalog_quick_edit/catalog__product__', 'user_token=' . $this->session->data['user_token'] . $url, true));
			}
		}
	...
	protected function getList() {
		$data['module_catalog_quick_edit_tooltip'] = ($this->config->get('module_catalog_quick_edit_quick_edit_on') == 'dblclick') ? $this->language->get('text_double_click_edit') : $this->language->get('text_click_edit');
		$data['module_catalog_quick_edit_quick_edit_on'] = $this->config->get('module_catalog_quick_edit_quick_edit_on');
		$data['module_catalog_quick_edit_row_hover_highlighting'] = $this->config->get('module_catalog_quick_edit_row_hover_highlighting');
		$data['module_catalog_quick_edit_alternate_row_colour'] = $this->config->get('module_catalog_quick_edit_alternate_row_colour');
		$data['module_catalog_quick_edit_list_view_image_width'] = $this->config->get('module_catalog_quick_edit_list_view_image_width');
		$data['module_catalog_quick_edit_list_view_image_height'] = $this->config->get('module_catalog_quick_edit_list_view_image_height');

		$this->document->addScript('view/javascript/quickedit/catalog.min.js');

		$this->document->addStyle('view/stylesheet/quickedit/catalog.min.css');
		...
		$filters['sub_category'] = (isset($this->request->get['filter_sub_category'])) ? $this->request->get['filter_sub_category'] : $this->config->get('module_catalog_quick_edit_catalog_products_filter_sub_category');
		$filters['special_price'] = (isset($this->request->get['filter_special_price'])) ? $this->request->get['filter_special_price'] : '';

		if (isset($this->request->get['sort'])) {
			$sort = $this->request->get['sort'];
		} else {
			$sort = $this->config->get('module_catalog_quick_edit_catalog_products_default_sort');
		}

		if (isset($this->request->get['order'])) {
			$order = $this->request->get['order'];
		} else {
			$order = $this->config->get('module_catalog_quick_edit_catalog_products_default_order');
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href'      => (int)$this->config->get('module_catalog_quick_edit_override_menu_entry') ? $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . $url . '&aqer=1', true) : $this->url->link('extension/module/catalog_quick_edit/catalog__product__', 'user_token=' . $this->session->data['user_token'] . $url, true),
			'active'    => true
		);
		...
		$actions = $this->config->get('module_catalog_quick_edit_catalog_products_actions');
		...
		$columns = $this->config->get('module_catalog_quick_edit_catalog_products');
		...
		$results = $this->model_extension_module_quickedit_catalog_product->getProducts($filter_data);

		$product_total = $this->model_extension_module_quickedit_catalog_product->getTotalProducts();
				...
				foreach ($columns as $column => $attr) {
					if ($column == 'image') {
						if ($result['image'] && file_exists(DIR_IMAGE . $result['image'])) {
							$image = $this->model_tool_image->resize($result['image'], $this->config->get('module_catalog_quick_edit_list_view_image_width'), $this->config->get('module_catalog_quick_edit_list_view_image_height'));
						} else {
							$image = $this->model_tool_image->resize('no_image.png', $this->config->get('module_catalog_quick_edit_list_view_image_width'), $this->config->get('module_catalog_quick_edit_list_view_image_height'));
						}
						...
						if ((int)$result['status'] || !$this->config->get('module_catalog_quick_edit_highlight_status')) {
							$row[$column] = ((int)$result['status'] ? $this->language->get('text_enabled') : $this->language->get('text_disabled'));
						} else {
							$row[$column] = ((int)$result['status'] ? $this->language->get('text_enabled') : '<span class="label label-danger">' . $this->language->get('text_disabled') . '</span>');
						}
		...
		$data['update_url'] = html_entity_decode($this->url->link('extension/module/catalog_quick_edit/catalog__product__quick_update', 'user_token=' . $this->session->data['user_token'], true));
		$data['refresh_url'] = html_entity_decode($this->url->link('extension/module/catalog_quick_edit/catalog__product__refresh_data', 'user_token=' . $this->session->data['user_token'], true));
		$data['load_popup_url'] = html_entity_decode($this->url->link('extension/module/catalog_quick_edit/catalog__product__load_popup', 'user_token=' . $this->session->data['user_token'], true));
		...
		$data['single_lang_editing'] = $this->config->get('module_catalog_quick_edit_single_language_editing') || ((int)$lang_count == 1);
		$data['batch_edit'] = (int)$this->config->get('module_catalog_quick_edit_batch_edit');

		if (in_array("filter", $displayed_columns)) {
			$this->load->model('extension/module/quickedit/catalog/filter');
			$data['_filters'] = $this->model_extension_module_quickedit_catalog_filter->getFiltersByGroup();
		}
		...
		foreach ($this->config->get('module_catalog_quick_edit_catalog_products') as $column => $attr) {
			if ((int)$this->config->get('module_catalog_quick_edit_override_menu_entry')) {
				...
				$data['sorts'][$column] = $this->url->link('extension/module/catalog_quick_edit/catalog__product__', 'user_token=' . $this->session->data['user_token'] . '&sort=' . $attr['sort'] . $url, true);
			}
		}
		...
		if ((int)$this->config->get('module_catalog_quick_edit_override_menu_entry')) {
			...
			$pagination->url = $this->url->link('extension/module/catalog_quick_edit/catalog__product__', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
		}
		...
		$template = 'extension/module/quickedit/catalog/product_list';
	...
	public function filter() {
		...
		$results = $this->model_extension_module_quickedit_catalog_filter->getFiltersByFilterGroupId($filter_group_id);
	...
	public function load_popup() {
		...
		$template = 'extension/module/quickedit/catalog/product_quick_form';
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			foreach ((array)$id as $_id) {
				if ($this->model_extension_module_quickedit_catalog_product->quickEditProduct($_id, $column, $value, $lang_id, $this->request->post)) {
					$results['done'][] = $_id;
				} else {
					$results['failed'][] = $_id;
				}
			...
			if ($results['done']) {
				...
				if (in_array($column, array('model', 'sku', 'upc', 'location', 'attributes', 'discounts', 'images', 'options', 'recurrings', 'related', 'specials', 'descriptions'))) {
					...
					if ((int)$value || !$this->config->get('module_catalog_quick_edit_highlight_status')) {
						$response['value'] = ((int)$value) ? $this->language->get('text_enabled') : $this->language->get('text_disabled');
					} else {
						$response['value'] = ((int)$value) ? $this->language->get('text_enabled') : '<span class="label label-danger">' . $this->language->get('text_disabled') . '</span>';
					}
					$response['values']['*'] = $response['value'];
				} else if ($column == 'image') {
					$this->load->model('tool/image');
					if ($value && file_exists(DIR_IMAGE . $value)) {
						$image = $this->model_tool_image->resize($value, $this->config->get('module_catalog_quick_edit_list_view_image_width'), $this->config->get('module_catalog_quick_edit_list_view_image_height'));
					} else {
						$image = $this->model_tool_image->resize('no_image.png', $this->config->get('module_catalog_quick_edit_list_view_image_width'), $this->config->get('module_catalog_quick_edit_list_view_image_height'));
					}
					foreach ($id as $_id) {
	...
	private function validatePermissions() {
		if (!$this->user->hasPermission('modify', 'catalog/product') || !$this->user->hasPermission('modify', 'extension/module/catalog_quick_edit')) {
