<?php
class ControllerExtensionModuleQuickeditCatalogFilter extends Controller {
	...
	public function index() {
		...
		$this->load->language('extension/module/quickedit/catalog/filter');
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			...
			foreach($this->config->get('module_catalog_quick_edit_catalog_filters') as $column => $attr) {
			...
			if ((int)$this->config->get('module_catalog_quick_edit_override_menu_entry')) {
				...
				$this->response->redirect($this->url->link('extension/module/catalog_quick_edit/catalog__filter__', 'user_token=' . $this->session->data['user_token'] . $url, true));
			}
		}
	...
	protected function getList() {
		...
		if (isset($this->request->get['sort'])) {
			...
			$sort = $this->config->get('module_catalog_quick_edit_catalog_filters_default_sort');
		}

		if (isset($this->request->get['order'])) {
			$order = $this->request->get['order'];
		} else {
			$order = $this->config->get('module_catalog_quick_edit_catalog_filters_default_order');
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href'      => (int)$this->config->get('module_catalog_quick_edit_override_menu_entry') ? $this->url->link('catalog/filter', 'user_token=' . $this->session->data['user_token'] . $url . '&aqer=1', true) : $this->url->link('extension/module/catalog_quick_edit/catalog__filter__', 'user_token=' . $this->session->data['user_token'] . $url, true),
			'active'    => true
		);
		...
		$columns = $this->config->get('module_catalog_quick_edit_catalog_filters');
		...
		$results = $this->model_extension_module_quickedit_catalog_filter->getFilterGroups($filter_data);

		$filter_total = $this->model_extension_module_quickedit_catalog_filter->getTotalFilterGroups();
		...
		foreach ($results as $result) {
			...
			if (!is_array($columns)) {
				...
				foreach ($columns as $column => $attr) {
					// if ($column == 'filter') {
					// 	$fs = $this->model_extension_module_quickedit_catalog_filter->getFiltersByFilterGroupId($result['filter_group_id']);
					// 	$filters = array();
		...
		$data['update_url'] = html_entity_decode($this->url->link('extension/module/catalog_quick_edit/catalog__filter__quick_update', 'user_token=' . $this->session->data['user_token'], true));
		$data['refresh_url'] = html_entity_decode($this->url->link('extension/module/catalog_quick_edit/catalog__filter__refresh_data', 'user_token=' . $this->session->data['user_token'], true));
		$data['load_popup_url'] = html_entity_decode($this->url->link('extension/module/catalog_quick_edit/catalog__filter__load_popup', 'user_token=' . $this->session->data['user_token'], true));
		...
		foreach ($this->config->get('module_catalog_quick_edit_catalog_filters') as $column => $attr) {
			if ((int)$this->config->get('module_catalog_quick_edit_override_menu_entry')) {
				...
				$data['sorts'][$column] = $this->url->link('extension/module/catalog_quick_edit/catalog__filter__', 'user_token=' . $this->session->data['user_token'] . '&sort=' . $attr['sort'] . $url, true);
			}
		}
		...
		if ((int)$this->config->get('module_catalog_quick_edit_override_menu_entry')) {
			...
			$pagination->url = $this->url->link('extension/module/catalog_quick_edit/catalog__filter__', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
		}
		...
		$template = 'extension/module/quickedit/catalog/filter_list';
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateRefreshData($this->request->post)) {
			...
			foreach ($this->request->post['data'] as $column => $filters) {
				foreach ($filters as $id) {
					switch ($column) {
						case 'filter':
							$fs = $this->model_extension_module_quickedit_catalog_filter->getFiltersByFilterGroupId($id);
							$_filters = array();
							foreach($fs as $filter) {
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			foreach ((array)$id as $_id) {
				if ($this->model_extension_module_quickedit_catalog_filter->quickEditFilter($_id, $column, $value, $lang_id, $this->request->post)) {
					$results['done'][] = $_id;
				} else {
					$results['failed'][] = $_id;
				}
	...
	private function validatePermissions() {
		if (!$this->user->hasPermission('modify', 'catalog/filter') || !$this->user->hasPermission('modify', 'extension/module/catalog_quick_edit')) {
