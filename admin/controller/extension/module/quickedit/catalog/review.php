<?php
class ControllerExtensionModuleQuickeditCatalogReview extends Controller {
	...
	public function index() {
		...
		$this->load->model('extension/module/quickedit/catalog/review');

		$this->load->language('catalog/review');
		$this->load->language('extension/module/quickedit/catalog/general');
		$this->load->language('extension/module/quickedit/catalog/review');
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			...
			foreach($this->config->get('module_catalog_quick_edit_catalog_reviews') as $column => $attr) {
			...
			if ((int)$this->config->get('module_catalog_quick_edit_override_menu_entry')) {
				...
				$this->response->redirect($this->url->link('extension/module/catalog_quick_edit/catalog__review__', 'user_token=' . $this->session->data['user_token'] . $url, true));
			}
		}
	...
	protected function getList() {
		...
		if (isset($this->request->get['sort'])) {
			...
			$sort = $this->config->get('module_catalog_quick_edit_catalog_reviews_default_sort');
		}

		if (isset($this->request->get['order'])) {
			$order = $this->request->get['order'];
		} else {
			$order = $this->config->get('module_catalog_quick_edit_catalog_reviews_default_order');
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href'      => (int)$this->config->get('module_catalog_quick_edit_override_menu_entry') ? $this->url->link('catalog/review', 'user_token=' . $this->session->data['user_token'] . $url . '&aqer=1', true) : $this->url->link('extension/module/catalog_quick_edit/catalog__review__', 'user_token=' . $this->session->data['user_token'] . $url, true),
			'active'    => true
		);
		...
		$columns = $this->config->get('module_catalog_quick_edit_catalog_reviews');
		...
		$results = $this->model_extension_module_quickedit_catalog_review->getReviews($filter_data);

		$review_total = $this->model_extension_module_quickedit_catalog_review->getTotalReviews();
		...
		$data['update_url'] = html_entity_decode($this->url->link('extension/module/catalog_quick_edit/catalog__review__quick_update', 'user_token=' . $this->session->data['user_token'], true));
		$data['refresh_url'] = html_entity_decode($this->url->link('extension/module/catalog_quick_edit/catalog__review__refresh_data', 'user_token=' . $this->session->data['user_token'], true));
		$data['load_popup_url'] = html_entity_decode($this->url->link('extension/module/catalog_quick_edit/catalog__review__load_popup', 'user_token=' . $this->session->data['user_token'], true));
		...
		foreach ($this->config->get('module_catalog_quick_edit_catalog_reviews') as $column => $attr) {
			if ((int)$this->config->get('module_catalog_quick_edit_override_menu_entry')) {
				...
				$data['sorts'][$column] = $this->url->link('extension/module/catalog_quick_edit/catalog__review__', 'user_token=' . $this->session->data['user_token'] . '&sort=' . $attr['sort'] . $url, true);
			}
		}
		...
		if ((int)$this->config->get('module_catalog_quick_edit_override_menu_entry')) {
			...
			$pagination->url = $this->url->link('extension/module/catalog_quick_edit/catalog__review__', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
		}
		...
		$template = 'extension/module/quickedit/catalog/review_list';
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			foreach ((array)$id as $_id) {
				if ($this->model_extension_module_quickedit_catalog_review->quickEditReview($_id, $column, $value, $lang_id, $this->request->post)) {
					$results['done'][] = $_id;
				} else {
					$results['failed'][] = $_id;
				}
	...
	private function validatePermissions() {
		if (!$this->user->hasPermission('modify', 'catalog/review') || !$this->user->hasPermission('modify', 'extension/module/catalog_quick_edit')) {
