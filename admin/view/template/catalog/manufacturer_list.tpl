            ...
            <table class="table table-bordered table-hover">
              <thead>
                <tr>
                  ...
                  <td class="text-center">Изоброжение</td>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="text-center">
                    <a onclick="upload_man('<?php echo $manufacturer['manufacturer_id']; ?>'); return false;" class="img-thumbnail">
                      <img id="img-<?php echo $manufacturer['manufacturer_id']; ?>" src="<?php echo $manufacturer['image_th']; ?>" alt="<?php echo $manufacturer['name']; ?>" data-placeholder="<?php echo $placeholder; ?>" class="img-thumbnail" /></a>
                    <input class="img_hidden" type="hidden" name="image_dod_<?php echo $manufacturer['manufacturer_id']; ?>" value="<?php echo $manufacturer['image']; ?>" id="input-image-dod-<?php echo $manufacturer['manufacturer_id']; ?>" />
                  </td>
...
<script>
  function upload_man(product_id) {
    $('#modal-image').remove();

    $.ajax({
      url: 'index.php?route=common/filemanager&token=<?php echo $token; ?>&thumb=dummy&target=input-image-dod-'+product_id,
      dataType: 'html',
      product: product_id,
      success: function(html) {
        ...
        $('#modal-image').modal('show').on('hide.bs.modal', function () {
          var el = $('#input-image-dod-'+product_id);
          update_image(product_id, el.val());
          el.val('');
        })
      }
    });
  ...
  function update_image(id, value){
    $.ajax({
      url: 'index.php?route=catalog/manufacturer/upload&token=<?php echo $token; ?>'+'&id=' + id + '&image='+value,
      type: 'get',
      dataType: 'json',
      success: function(json) {
        if(json['success'] == 1) {
          $('#img-'+id).attr("src", json['thumb']);
        }
      }
  });
  }
