...
$('body').on('click', '.btn-add-product-option-value', function(e) {
  ...
  var html = '';
	...
	html += '<span class="fa fa-close btn-remove-product-option-value" data-toggle="tooltip" title="<?= $button_remove ?>"></span>';

	html += '<div class="le-row">';
	html += '<label class="le-label _left" for="input-product-option-value-' + productId + '-' + optionRow + '-' + optionValueRow + '"><?= $entry_option_value ?></label>';
  ...
  html += '</select>';
	...
	html += '<div class="le-row">';
	html += '<label class="le-label _left" for="input-quantity-' + productId + '-' + optionRow + '-' + optionValueRow + '"><?= $entry_quantity ?></label>';
	html += '<input type="text" id="input-quantity-' + productId + '-' + optionRow + '-' + optionValueRow + '" name="" value="" placeholder="<?= $entry_quantity ?>" class="le-value _simple-value-2 _right" data-field="quantity" />';
	html += '</div>';

	html += '<div class="le-row">';
	html += '<label class="le-label _left" for="input-subtract-' + productId + '-' + optionRow + '-' + optionValueRow + '" style="padding-top: none;"><?= $entry_subtract ?></label>';
	...
	html += '<input id="input-price-' + productId + '-' + optionRow + '-' + optionValueRow + '" type="text" name="price" value="" placeholder="<?= $entry_price ?>" class="le-value _simple-value-2 _right-half" data-field="price" />';
	...
	html += '<input id="input-points-' + productId + '-' + optionRow + '-' + optionValueRow + '" type="text" name="points" value="" placeholder="<?= $entry_option_points ?>" class="le-value _simple-value-2 _right-half" data-field="points" />';
	...
	html += '<input id="input-weight-' + productId + '-' + optionRow + '-' + optionValueRow + '" type="text" name="weight" value="" placeholder="<?= $entry_weight ?>" class="le-value _simple-value-2 _right-half" data-field="weight" />';
...
$('body').on('click', '.btn-add-option', function(e) {
  ...
  html += '<div class="pull-right"><a type="button" class="btn-remove-option" data-target="#option-row-' + productId + '-' + optionRow + '" data-toggle="tooltip" title="<?=$button_remove?>"><i class="fa fa-close"></i></a></div>';
  html += '<div class="le-row">';
  ...
  $(this).attr('data-option-row', optionRow);
	...
	$.ajax({
    url: 'index.php?route=extension/module/handy_product_manager/getOptionsList&token=<?=$token?>&product_id=' + productId,
    type: 'GET',
    dataType: 'json',
    ...
    success: function(json) {
      if ('success' === json['status']) {
        ...
        //html += '<option value="add_new_option"><?=$hpm_text_option_new?></option>';
        html += '</select>';
...
$('body').on('change', '.option-selector', function() {
	...
	html += '<div class="le-label _name"><a class="option-link" href="index.php?route=catalog/option/edit&token=<?=$token?>&option_id=' + optionId + '" target="_blank" data-toggle="tooltip" title="<?=$hpm_text_option_edit?>">' + $(this).children('option:selected').text() + '</a></div>';

	html += '<!-- reqiure -->';
	html += '<div class="option-require le-row">';
	html += '<label class="le-label _left" for="input-required-' + productId + '-' + optionRow + '"><?=$entry_required?></label>';
  ...
  var productOptionId = liveUpdateAjax(data, 'add_product_option');
	...
	if ('text' == aOptions[optionId]['type']) {
		html += '<div class="le-row">';
		html += '<label class="le-label" for="input-value-' + productId + '-' + optionRow + '"><?=$entry_option_value?></label>';
		html += '<input type="text" value="" placeholder="<?=$entry_option_value?>" id="input-value-' + productId + '-' + optionRow + '" class="le-value _simple-value" data-field="value" />';
		html += '</div>';
	}
	...
	if ('textarea' == aOptions[optionId]['type']) {
		html += '<div class="le-row">';
		html += '<label class="le-label" for="input-value-' + productId + '-' + optionRow + '"><?= $entry_option_value ?></label>';
		html += '<textarea rows="5" placeholder="<?= $entry_option_value ?>" id="input-value-' + productId + '-' + optionRow + '" class="le-value _simple-value _textarea" data-field="value"></textarea>';
		html += '</div>';
	}
	...
	if ('select' == aOptions[optionId]['type'] || 'radio' == aOptions[optionId]['type'] || 'checkbox' == aOptions[optionId]['type'] || 'image' == aOptions[optionId]['type']) {
		...
		html += '</div>';
    html += '<button type="button" data-toggle="tooltip" data-target="#option-values-' + productId + '-' + optionRow + '" title="<?=$button_option_value_add?>" class="btn btn-sm btn-primary btn-add-product-option-value"><i class="fa fa-plus-circle"></i></button>';
	}
