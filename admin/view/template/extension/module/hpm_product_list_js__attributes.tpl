...
$.ajax({
  url: 'index.php?route=extension/module/handy_product_manager/getAllAttributeValues&token=<?=$token?>',
  type: 'GET',
  dataType: 'json',
  async: false,
  success: function(json) {
    if ('success' === json['status']) {
      ...
      alert('error on extension/module/handy_product_manager/getAllAttributeValues');
    }
  },
...
$('body').on('click', '.btn-add-attribute', function(e) {
  ...
  html += '<div class="pull-right"><a type="button" class="btn-remove-attribute" data-target="#attribute-row-' + productId + '-' + attributeRow + '" data-toggle="tooltip" title="<?=$button_remove?>"><i class="fa fa-close"></i></a></div>';
  ...
  $.ajax({
    url: 'index.php?route=extension/module/handy_product_manager/getAttributeList&token=<?=$token?>&product_id=' + productId,
    type: 'GET',
    dataType: 'json',
    ...
    success: function(json) {
      if ('success' === json['status']) {
        ...
        html += '<option value=""><?=$hpm_text_attribute_select?></option>';
        for (var key in data) {
          html += '<option value="' + data[key]['attribute_id'] + '">' + data[key]['name'] + '</option>';
        }
        html += '<option value="add_new_attribute">! <?=$hpm_text_attribute_new?></option>';
        html += '</select>';
...
function liveUpdateAttributeSelector(identifierSelector, identifierContainer, identifierRow) {
  ...
  $('#' + identifierSelector).on('change', function(e){
    ...
    if ('add_new_attribute' === $('#' + identifierSelector).val()) {
      ...
      var newHtml = '<div class="le-label"><a href="<?=HTTPS_SERVER?>index.php?route=catalog/attribute/edit&token=<?=$token?>&attribute_id=' + attributeId + '" class="attribute-link" target="_blank">' + $('#' + identifierSelector + ' option:selected').text() + '</a>:</div>';
...
function buildNewAttributeInput(identifierContainer, productId, attributeRow) {
  ...
  html += '<span class=""><img src="language/<?=$language['code']?>/<?=$language['code']?>.png" title="<?=$language['name']?>" /></span>';
  html += '</div>';
  html += '<div class="le-lang-values-content" data-product-id="' + productId + '" data-language-id="<?=$language['language_id']?>">';
  html += '<input type="text" class="le-value _attribute-name le-text" name="attribute_description[<?=$language['language_id']?>]" value="" data-field="text" id="attribute-value-' + productId + '-' + attributeRow + '-<?=$language['language_id']?>" />';
  html += '</div>';
  <?php } ?>
  html += '</div>';
  html += '<button class="btn btn-sm btn-warning btn-save-new-attribute" data-product-id="' + productId + '" data-attribute-row="' + attributeRow + '"><i class="fa fa-save"></i> &nbsp; <?=$hpm_text_attribute_new_save?></button>';
  html += '</div>';

  $('#' + identifierContainer).append(html);

  $.ajax({
    url: 'index.php?route=extension/module/handy_product_manager/getAttributeGroupList&token=<?=$token?>',
    type: 'GET',
    dataType: 'json',
    beforeSend: function() { loaderOn(); },
    success: function(json) {
      if ('success' === json['status']) {
        ...
        html = '';
        var data = json['data'];
        html += '<option value=""><?=$hpm_text_attribute_group_select?></option>';
        for (var key in data) {
          html += '<option value="' + data[key]['attribute_group_id'] + '">' + data[key]['name'] + '</option>';
        }
...
$('body').on('click', '.btn-save-new-attribute', function(e) {
  ...
  <?php foreach ($languages as $language) { ?>
  if("" === $('#attribute-value-' + productId + '-' + attributeRow + '-<?=$language['language_id']?>').val()) {
    hasError = true;
    $('#attribute-value-' + productId + '-' + attributeRow + '-<?=$language['language_id']?>').addClass('is-error');
  }
  <?php } ?>
  ...
  if (hasError) {
    ...
    <?php foreach ($languages as $language) { ?>
    data += '&attribute_description[<?=$language['language_id']?>]=' + $('#attribute-value-' + productId + '-' + attributeRow + '-<?=$language['language_id']?>').val();
    <?php } ?>
      ...
      success: function(json) {
        ...
        if ('success' === json['status']) {
          ...
          $('#attribute-row-' + productId + '-' + attributeRow + '-attribute-form').html('<div class="le-label">'
            + '<a class="attribute-link" href="index.php?route=catalog/attribute/edit&token=<?=$token?>&attribute_id=' + attributeId + '" target="_blank">' + attributeName + '</a>'
            + ':</div>');

          addAttributeToProduct(attributeId, productId);

          setTimeout(function () {
            buildAttributeValuesOnAddNewAttribute('attribute-row-' + productId + '-' + attributeRow + '-attribute-form', attributeId);
            liveUpdateAttributeValueSelector();
          }, 100);
...
*/
  ...
  html += '<span class=""><img src="language/<?=$language['code']?>/<?=$language['code']?>.png" title="<?=$language['name']?>" /></span>';
  html += '</div>';
  html += '<div class="le-lang-values-content" data-product-id="' + productId + '" data-language-id="<?=$language['language_id']?>" data-attribute-id="' + attributeId + '">';
  html += '<input type="text" class="le-value attribute-value le-text" value="" data-field="text" id="attribute-value-' + productId + '-' + attributeRow + '-<?=$language['language_id']?>" />';
  html += '<select class="le-selector attribute-value-selector _live-added" data-target="#attribute-value-' + productId + '-' + attributeRow + '-<?=$language['language_id']?>" data-attribute-id="' + attributeId + '" data-language-id="<?=$language['language_id']?>" id="attribute-value-selector-' + productId + '-' + attributeRow + '-<?=$language['language_id']?>">';
