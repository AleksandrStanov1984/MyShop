<div class="modal-header">
  ...
  <h4 class="modal-title"><?php echo $tab_attribute; ?></h4>
</div>
<div class="modal-body" id="body-mass-product-attributes">
  <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-mass-product-attributes" class="form-horizontal">	
	<div class="form-group">
 	  <div class="col-sm-12	">
	    <table id="attribute" class="table table-striped table-bordered table-hover">
		  <thead>
		    <tr>
			  <td class="text-left" width="30%"><?php echo $entry_attribute; ?></td>
		  ...
		  <tbody>
		    <?php $attribute_row = 0; ?>
		    <tr id="attribute-row<?php echo $attribute_row; ?>"></tr>
		    <?php $attribute_row++; ?>
		  </tbody>
		  <tfoot>
		    <tr>
	...
	<div class="form-group">	
	  <label class="col-sm-3 control-label" for="input-attribute-method"><?php echo $entry_method_edit; ?></label>
	  <div class="col-sm-9">
		<select name="attribute_method_edit" id="input-attribute-method" class="form-control">
...
<div class="modal-footer">
  <div class="text-center"><a data-form="form-mass-product-attributes" class="button-save btn btn-info btn-lg"><?php echo $button_save; ?></a></div>
</div>
...
$(".button-save").on('click', function(){		
	$.ajax({
		...
		type: 'post',
		dataType: 'json',
		data: $('#product-list input[type=\'checkbox\']:checked, #' + $(this).data('form') + ' input[type=\'text\'], #' + $(this).data('form') + ' input[type=\'hidden\'], #' + $(this).data('form') + ' textarea, #' + $(this).data('form') + ' select'),
		success: function(json) {				
			if (json['error']) {
				$('#form-mass-product-attributes').before('<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> ' + json['error'] + '</div>');
				
				$('.alert-danger').delay(1500).fadeOut(500);
			}
								
			if (json['success']) {
				$('#form-mass-product-attributes').before('<div class="alert alert-success"><i class="fa fa-check-circle"></i> ' + json['success'] + '</div>');	
