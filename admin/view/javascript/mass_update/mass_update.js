...
function filter() {
	var url = 'index.php?route=mass_update/mass_update&token=' + getURLVar('token');
...
$(document).ready(function() {
	...
	$('input[name=\'filter_name\']').autocomplete({
		'source': function(request, response) {
			$.ajax({
				url: 'index.php?route=mass_update/mass_update/autocomplete&token=' + getURLVar('token') + '&filter_name=' +  encodeURIComponent(request),
				dataType: 'json',
	...
	$('input[name=\'filter_model\']').autocomplete({
		'source': function(request, response) {
			$.ajax({
				url: 'index.php?route=mass_update/mass_update/autocomplete_model&token=' + getURLVar('token') + '&filter_model=' +  encodeURIComponent(request),
				dataType: 'json',
				success: function(json) {
					response($.map(json, function(item) {
						return {
	...
	$('input[name=\'filter_sku\']').autocomplete({
		'source': function(request, response) {
			$.ajax({
				url: 'index.php?route=mass_update/mass_update/autocomplete_sku&token=' + getURLVar('token') + '&filter_sku=' +  encodeURIComponent(request),
				dataType: 'json',
				success: function(json) {
					response($.map(json, function(item) {
						return {
	...
	$('input[name=\'filter_location\']').autocomplete({
		'source': function(request, response) {
			$.ajax({
				url: 'index.php?route=mass_update/mass_update/autocomplete_location&token=' + getURLVar('token') + '&filter_location=' +  encodeURIComponent(request),
				dataType: 'json',
				success: function(json) {
					response($.map(json, function(item) {
						return {
	...
	$('input[name=\'filter_manufacturer\']').autocomplete({
		'source': function(request, response) {
			$.ajax({
				url: 'index.php?route=mass_update/mass_update/autocomplete_manufacturer&token=' + getURLVar('token') + '&filter_name=' +  encodeURIComponent(request),
				dataType: 'json',
				success: function(json) {					
					json.unshift({
						manufacturer_id: '*',
	...
	$('input[name=\'filter_attribute\']').autocomplete({
		'source': function(request, response) {
			$.ajax({
				url: 'index.php?route=catalog/attribute/autocomplete&token=' + getURLVar('token') + '&filter_name=' +  encodeURIComponent(request),
				dataType: 'json',
				success: function(json) {
					json.unshift({
						attribute_id: '*',
	...
	$('input[name=\'filter_filter\']').autocomplete({
		'source': function(request, response) {
			$.ajax({
				url: 'index.php?route=catalog/filter/autocomplete&token=' + getURLVar('token') + '&filter_name=' +  encodeURIComponent(request),
				dataType: 'json',
				success: function(json) {
					json.unshift({
						filter_id: '*',
	...
	$('.ajax-status').on('click', function() { 
		...
		var type = 'product_status';

		if (permission) {
			$.ajax({
				url: 'index.php?route=mass_update/product_mass/product&token=' + getURLVar('token') + '&type=' + type,
				type: 'get',
	...
	$('#top-mass-edit-data, #bottom-mass-edit-data').on('change', function() {
		...
		if (permission) {
			if ($('#product-list input[type=\'checkbox\']:checked').val()) {	
				...
				$('#modal-product-content').load('index.php?route=mass_update/product_mass/edit_mass_' + $(this).val() + '&token=' + getURLVar('token'));
			} else {
...
function massDelete(del_val) {
	...
	var type = 'mass_delete';
	
	if (permission) {
		if ($('#product-list input[type=\'checkbox\']:checked').val()) {	
			if (confirm(text_confirm)) {
				$.ajax({
					url: 'index.php?route=mass_update/product_mass/product&token=' + getURLVar('token') + '&del_val=' + del_val + '&type=' + type,
					type: 'post',
					dataType: 'json',
					...
					success: function(json) {
						...
						if (json['success']) {
							...
							if (del_val == 'name' || del_val == 'image' || del_val == 'model' || del_val == 'price' || del_val == 'quantity' || del_val == 'sort_order' || del_val == 'manufacturer' || del_val == 'category' || del_val == 'special' || del_val == 'products') {
								setTimeout(function(){
...
function editImage(product_id) {
	...
	var type = 'product_image';
	
	$.ajax({
		url: 'index.php?route=mass_update/product_mass/product&token=' + getURLVar('token') + '&product_id=' + product_id + '&image=' + image + '&type=' + type,
		type: 'post',
		dataType: 'json',
		success: function(json) {									
			if (json['error']) {								
			...
			if (json['success']) {
				...
				$('#product-image' + product_id).load('index.php?route=mass_update/product_mass/loadImage&token=' + getURLVar('token') + '&product_id=' + product_id);
...
function getName(product_id) {
	$('#product-name-content-' + product_id).load('index.php?route=mass_update/product_mass/getProductName&token=' + getURLVar('token') + '&product_id=' + product_id);
}

function getCategory(product_id) {
	$('#product-category-content-' + product_id).load('index.php?route=mass_update/product_mass/getProductCategory&token=' + getURLVar('token') + '&product_id=' + product_id);
}

function getManufacturer(product_id) {
	$('#product-manufacturer-content-' + product_id).load('index.php?route=mass_update/product_mass/getProductManufacturer&token=' + getURLVar('token') + '&product_id=' + product_id);
}

function editModel(product_id) {
	var model = $('#input-model' + product_id).val();
	var without = $('#text-without').val();
	var type = 'product_model';
							
	$.ajax({
		url: 'index.php?route=mass_update/product_mass/product&token=' + getURLVar('token') + '&product_id=' + product_id + '&model=' + model + '&type=' + type,
		type: 'post',
		dataType: 'json',
		success: function(json) {									
			if (json['error']) {								
...
function editQuantity(product_id) {
	...
	var type = 'product_quantity';
							
	$.ajax({
		url: 'index.php?route=mass_update/product_mass/product&token=' + getURLVar('token') + '&product_id=' + product_id + '&quantity=' + quantity + '&type=' + type,
		type: 'post',
		dataType: 'json',
		success: function(json) {									
...
function editPrice(product_id) {
	...
	var type = 'product_price';
							
	$.ajax({
		url: 'index.php?route=mass_update/product_mass/product&token=' + getURLVar('token') + '&product_id=' + product_id + '&price=' + price + '&type=' + type,
		type: 'post',
		dataType: 'json',
		success: function(json) {								
			if (json['error']) {								
			...
			if (json['success']) {
				...
				$('#product-price' + product_id).load('index.php?route=mass_update/product_mass/loadPrice&token=' + getURLVar('token') + '&product_id=' + product_id);	
...
function editSort(product_id) {
	var sort_order = $('#input-sort' + product_id).val();
	var type = 'product_sort_order';
							
	$.ajax({
		url: 'index.php?route=mass_update/product_mass/product&token=' + getURLVar('token') + '&product_id=' + product_id + '&sort_order=' + sort_order + '&type=' + type,
		type: 'post',
		dataType: 'json',
		success: function(json) {									
			if (json['error']) {								
			...
			if (json['success']) {
				...
				if (sort_order != '') {
					$('#product-sort' + product_id).html(sort_order);
				} else {
					$('#product-sort' + product_id).html('0');
				}

				$('#close-sort-' + product_id).trigger('click');
			}
		}
	});
}

function sortClear(product_id) {
	$('#input-sort' + product_id).val('');
}
...
function editMore(product_id, val) {	
	...
	$('#modal-product-content').load('index.php?route=mass_update/product_mass/edit_' + val + '&token=' + getURLVar('token') + '&product_id=' + product_id);
}
...
function editProductSetting(val) {
	$('#modal-product-edit').modal('show');
	...
	$('#modal-product-content').load('index.php?route=mass_update/product_mass/product_settings_' + val + '&token=' + getURLVar('token'));
}