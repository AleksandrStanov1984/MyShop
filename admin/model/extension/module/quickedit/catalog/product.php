<?php
class ModelExtensionModuleQuickeditCatalogProduct extends Model {
...
//		}
		...
		// Actions
		if ((int)$this->config->get('module_catalog_quick_edit_highlight_actions')) {
...
//			'weight_class'      => 'p.weight_class_id',
		...
		foreach ($date_filters as $key => $value) {
			if (isset($data["filter_$key"]) && !is_null($data["filter_$key"])) {
				if ($this->config->get('module_catalog_quick_edit_interval_filter')) {
		...
		foreach ($anywhere_filters as $key => $value) {
			if (!empty($data["filter_$key"])) {
				if ($this->config->get('module_catalog_quick_edit_match_anywhere')) {
	...
	public function quickEditProduct($product_id, $column, $value, $lang_id=null, $data=null) {
        $editable = array('manufacturer', 'image', 'model', 'sku', 'upc', 'ean', 'jan', 'mpn', 'isbn', 'location', 'quantity', 'price', 'weight', 'status', 'sort_order', 'tax_class', 'minimum', 'subtract', 'stock_status', 'shipping', 'date_available', 'date_added', 'length', 'width', 'height', 'points', 'viewed');
		...
		if (in_array($column, $editable)) {
            ...
            else if (in_array($column, array('manufacturer', 'tax_class', 'stock_status')))
