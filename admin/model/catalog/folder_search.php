...
class ModelCatalogFolderSearch extends Model {
	...
	public function createTable(){
		...
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "oc_tag`;");
		$this->db->query("CREATE TABLE `" . DB_PREFIX . "tag` (
	...
	public function getFirstNoCodeProduct(){
		
		$res = $this->db->query("SELECT distinct(pd.product_id) FROM `" . DB_PREFIX . "product_description` pd
									LEFT JOIN `" . DB_PREFIX . "product` p ON p.product_id = pd.product_id
									WHERE pd.product_id NOT IN (SELECT p1.product_id FROM `" . DB_PREFIX . "product_description_search` p1)
									AND p.status = 1
									LIMIT 100;");
	...
	public function deleteAllDescription(){
		...
		$this->db->query("DELETE FROM `" . DB_PREFIX . "product_description_search`;");
		
	}	
	public function deleteDescription($product_id){
		
		if((int)$product_id > 0){
			$this->db->query("DELETE FROM `" . DB_PREFIX . "product_description_search` WHERE product_id='".(int)$product_id."';");
		}
