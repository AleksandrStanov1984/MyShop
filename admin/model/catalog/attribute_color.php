<?php
class ModelCatalogAttributeColor extends Model {

	
	public function groupAttributeColorValue(){
		
		$sql = "SELECT * FROM " . DB_PREFIX . "attribute_value 
				WHERE attribute_id = 15 ORDER BY language_id ASC";
	...
	public function getColorInfo($color){
		
		$sql = "SELECT * FROM " . DB_PREFIX . "attribute_color WHERE text = '".$this->db->escape($color)."'";
			...
			foreach ($attribute_description->rows as $rows) {
				...
				foreach ($rows2 as $row) {
					if($rows['language_id'] == 1){
						$key = $row;
						//$sort[mb_convert_case($row, MB_CASE_TITLE, 'UTF-8')] = md5(utf8_strtoupper($key));
                        $sort[$row] = md5($key);
					}
					
					//$product_attribute_data[md5(utf8_strtoupper($key))][$rows['language_id']] = mb_convert_case($row,MB_CASE_TITLE, 'UTF-8');
