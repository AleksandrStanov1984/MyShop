<?php
class ModelMassUpdateMassUpdate extends Model {
	public function addProduct($data) {
		...
		if (isset($this->request->post['auto_keyword_product'])) {
			if ($data['keyword']) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'product_id=" . (int)$product_id . "', keyword = '" . $this->db->escape($data['keyword']) . "'");
			} else {
				$this->load->model('mass_update/product_mass');
				if ($productName) {
					$this->model_mass_update_product_mass->generateProductSeoKeyword($product_id, $productName, $this->config->get('mass_update_seo_template_url_alias'), $this->config->get('config_language'));
				}
			}
		} else {
			if ($data['keyword']) {
	...
	public function editProduct($product_id, $data) {
		...
		if ($this->request->post['auto_keyword_product']) {
			if ($data['keyword']) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'product_id=" . (int)$product_id . "', keyword = '" . $this->db->escape($data['keyword']) . "'");
			} else {
				$this->load->model('mass_update/product_mass');
	...
	public function copyCategoryProducts($category_new_id, $new_category_id) {
		$query = $this->db->query("SELECT DISTINCT product_id FROM " . DB_PREFIX . "product_to_category WHERE category_id = '" . $category_new_id . "'");

		foreach ($query->rows as $product) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_to_category SET product_id = '" . (int)$product['product_id'] . "', category_id = '" . (int)$new_category_id . "'");
		}
	}
	...
	public function getProducts($data = array()) {
		$sql = "SELECT p.*, pd.*,  m.name as 'manufacturer' FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)";
