...
class ModelToolSimpleApiCustom extends Model {
	...
	public function getProductPickupPoints() {
		$query = $this->db->query("SELECT productpickuppoint_id AS 'id', CONCAT(name, ': ', address) AS 'text' FROM sz_productpickuppoint WHERE active = true ORDER BY id");
        return $query->rows;
	}
}