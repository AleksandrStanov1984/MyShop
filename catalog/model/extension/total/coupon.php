...
class ModelExtensionTotalCoupon extends Model {
	...
	public function getProductPickupPointCoupon($productPickupPointId) {
		if(isset($productPickupPointId) && $productPickupPointId > 0) {
			$coupon_query = $this->db->query("SELECT cpn.code AS couponCode FROM `sz_productpickuppoint` AS ppp INNER JOIN `oc_coupon` AS cpn ON cpn.coupon_id = ppp.coupon_id WHERE ppp.productpickuppoint_id = ".$productPickupPointId);
			if ($coupon_query->num_rows) {
				return $coupon_query->row['couponCode'];
			}
		}
	...
	public function getProductPickupPoint($productPickupPointId) {
		if(isset($productPickupPointId) && $productPickupPointId > 0) {
			$ppp_query = $this->db->query("SELECT address FROM `sz_productpickuppoint` WHERE productpickuppoint_id = ".$productPickupPointId);
			if ($ppp_query->num_rows) {
				return $ppp_query->row['address'];
			}
		}
