...
class ModelAccountCustomer extends Model {
	...
	public function getCustomerByEmailOrPhone($email) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE LOWER(email) LIKE '%" . $this->db->escape(utf8_strtolower($email)) . "%' OR LOWER(telephone) LIKE '" . $this->db->escape($email) . "'");
	...
	public function getCustomerEmail($customer_id) {
		...
		return isset($query->row['email']) ? $query->row['email'] : '';
	}
	...
	public function addCode($login, $code) {
		$query = $this->db->query("SELECT * FROM `customer_login` WHERE login = '" . $this->db->escape($login) . "'");

		if($query->row) {
			$this->db->query("UPDATE `customer_login` SET code = '" . (int)$code . "' WHERE login = '" . $login . "'");
		} else {
			$this->db->query("INSERT INTO `customer_login` SET code = '" . (int)$code . "', login = '" . $login . "'");
		}
		
	}

	public function getCode($login) {
		$query = $this->db->query("SELECT code FROM `customer_login` WHERE login = '" . $this->db->escape($login) . "'");
		return isset($query->row['code']) ? $query->row['code'] : false;
	}

	public function clearCode($phone) {
		$this->db->query("DELETE FROM `customer_login` WHERE login LIKE '" . $phone . "'");
	}

	public function approveCustomer($login) {
		$this->db->query("UPDATE `customer_login` SET entry = '1' WHERE login = '" . $login . "'");
		return true;
	}

	public function isLogin($login) {
		$query = $this->db->query("SELECT entry FROM `customer_login` WHERE login = '" . $this->db->escape($login) . "' AND entry = '1'");
		return $query->row;
	}
}
