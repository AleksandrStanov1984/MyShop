<?php
class ControllerEventBrokenLinks extends Controller {
	public function index(&$view, &$data, &$output) {
		if (!strstr($view,'error/not_found')) return;
		if ( !$this->config->get('broken_links_enable') )
			return;
		...
		if(isset($link_isset)) unset($link_isset);

		if (isset($this->request->server['REQUEST_URI'])) {
			$request_uri = $this->config->get('config_url') . ltrim($this->request->server['REQUEST_URI'],'/');

			$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "broken_notfound` WHERE `request_uri` = '" .  $this->db->escape($request_uri) . "'");	
			if($query->rows) $link_isset = true;	
		} else {
		...
		if(!isset($link_isset) && $referer != 'Referer not detected') {
			$this->db->query("
