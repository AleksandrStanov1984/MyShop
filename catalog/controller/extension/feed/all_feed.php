...
class ControllerExtensionFeedAllFeed extends Controller
{
    private $shop = array();
    ...
    public function index()
    {
        if ($this->config->get('rozetka_status')) {
            ...
            $this->load->model('export/all_feed');
            ...
            $allowed_type = array('rozetka', 'prom', 'hotline');
            if( isset($this->request->get['type']) && in_array($this->request->get['type'], $allowed_type) ) {
                $type = $this->request->get['type'];
            } else {
                exit('Указан неверный тип фида');
            }
            ...
            // Категории
            if($type == 'hotline') {
                $this->setHotlineCategoriesTree();
            } else {
                $allowed_categories_array = $this->model_export_all_feed->getProductCategories($out_of_stock_id, $vendor_required, $type);
                $allowed_categories = implode(',', $allowed_categories_array);

                $categories = $this->model_export_all_feed->getCategory($allowed_categories);
                $parent_categories = array();
                foreach ($categories as $category) {
                    if($category['parent_id']) $parent_categories[] = $category['parent_id'];
                    $this->setCategory(trim($category['name']), $category['category_id'], $category['parent_id']);
                }

                // check empty parents ids category
                $parent_categories = array_unique($parent_categories);

                $diff_categories_array = array_diff($parent_categories, $allowed_categories_array);

                if($diff_categories_array) $diff_categories = implode(',', $diff_categories_array);

                $categories = $this->model_export_all_feed->getCategory($diff_categories);
                foreach ($categories as $category) {
                    $this->setCategory(trim($category['name']), $category['category_id'], $category['parent_id']);
                }
            }
            ...
            // image setting
            $imageFolder = realpath(DIR_IMAGE . "rozetka");
            $imageAvailableExt = ['jpg', 'png'];
            $imageAvailableExtPattern = implode('|', $imageAvailableExt);
            
            $products = $this->model_export_all_feed->getProduct($out_of_stock_id, $vendor_required, $type);
            ...
            $setting = array(
                ...
                'offers_currency'   => $offers_currency,
                'decimal_place'     => $decimal_place,
                'image_folder'      => $imageFolder,
                'image_available_ext'=> $imageAvailableExt,
                'image_ext_pattern' => $imageAvailableExtPattern
            );

            if($type == 'rozetka') {
                $this->rozetkaTemplate($products, $setting);
            } elseif($type == 'prom') {
                $this->promTemplate($products, $setting);
            } elseif($type == 'hotline') {
                $this->hotlineTemplate($products, $setting);
            }   
            
            $this->response->addHeader('Content-Type: text/xml; charset=windows-1251');
            $this->response->setOutput($this->getYml($case));
            
        }
    }

    private function setHotlineCategoriesTree() {

        $this->load->model('export/all_feed');
        $categories = $this->model_export_all_feed->getCategoriesHotline();

        foreach ($categories as $category) {
            $this->setCategory(trim($category['name']), $category['category_id']);
        }
    ...
    private function rozetkaTemplate($products, $setting=array()) {
        ...
        foreach ($products as $product) {
            ...
            $data['available'] = ($product['quantity'] > 0 || $product['stock_status_id'] == $setting['in_stock_id']);
            ...
            if ((int)$product['price_special_rozetka'] > 0) {
                if ((int)$this->config->get('rozetka_percent_markup') > 0) {
                    ...
                    ), $setting['shop_currency'], $setting['offers_currency']), $setting['decimal_place'], '.', '');
            ...
            $data['currencyId'] = $this->config->get('rozetka_currency');
            ...
            // image
            $productImageFolder = "{$setting['image_folder']}/{$product['sku']}";

            if (is_dir($productImageFolder)) {
                $files = glob("{$productImageFolder}/*\.???");
                $images = [];
                foreach ($files as $file) {
                    if (in_array($ext = substr($file, -3), $setting['image_available_ext'])) {
                        $f = 'array_push';
                        if (preg_match('/\/1\.(' . $setting['image_ext_pattern'] . ')$/um', $file)) {
                            $f = 'array_unshift';
                        }
                ...
                if(!empty($images)) {
                    ...
                    foreach ($images as $image) {
                        if($j < 8) {
                            $res_images[] = $this->model_tool_image->resize($image['image'], 600, 500);
                            $j++;
                        } 
                    }
                    $data['picture'] = $res_images;
            ...
            if ($attribute_groups) {
                ...
                foreach ($attribute_groups as $attribute_group) {
                    ...
                    foreach ($attribute_group['attribute'] as $attribute) {
                        if($attribute['name'] == 'Высота (см)' || $attribute['name'] == 'Ширина (см)') {
    ...
    private function promTemplate($products, $setting=array()) {
        $this->load->model('catalog/product');

        foreach ($products as $product) {

            // оставляем только те товары у которых stock_status_id = 7
            if($product['stock_status_id'] != $setting['in_stock_id']) continue;
            ...
            if ((int)$this->config->get('rozetka_percent_markup') > 0) {
                ...
                $markup = $product['price'] * $this->config->get('rozetka_percent_markup') / 100;
            ...
            if (!is_null($product['special_price'])) {
                ...
                if ((int)$this->config->get('rozetka_percent_special_markup') > 0) {
                    ...
                    $sMarkup = $product['special_price'] * $this->config->get('rozetka_percent_special_markup') / 100;

                }

                $product['special_price'] = round((float)$product['special_price'] + (float)$sMarkup, (int)$this->config->get('rozetka_markup_round_decimals'));

            }

            $product['price'] = round((float)$product['price'] + (float)$markup, (int)$this->config->get('rozetka_markup_round_decimals'));
            $data = array();

            // Атрибуты товарного предложения
            $data['id'] = $product['product_id'];
            $data['available'] = ($product['stock_status_id'] == $setting['in_stock_id'] && (int)$product['status'] == 1);

            // Параметры товарного предложения
            $data['url'] = $this->url->link('product/product', 'path=' . $this->getPath($product['category_id']) . '&product_id=' . $product['product_id']);
            if (!is_null($product['special_price'])) {
                $data['price'] = number_format($this->currency->convert($this->tax->calculate($product['special_price'], $product['tax_class_id']), $setting['shop_currency'], $setting['offers_currency']), $setting['decimal_place'], '.', '');
                $data['oldprice'] = number_format($this->currency->convert($this->tax->calculate($product['price'], $product['tax_class_id']), $setting['shop_currency'], $setting['offers_currency']), $setting['decimal_place'], '.', '');
            } else {
                $data['price'] = number_format($this->currency->convert($this->tax->calculate($product['price'], $product['tax_class_id']), $setting['shop_currency'], $setting['offers_currency']), $setting['decimal_place'], '.', '');
            }
            $data['currencyId'] = $setting['offers_currency'];
            $data['categoryId'] = $product['category_id'];

            if($product['meta_h1']) {
                $h1_name = $this->clearName($product['meta_h1']);
                $data['name'] = $h1_name;
            } else {
                $name = $this->clearName($product['name']);
                $data['name'] = $name;
            }
            ...
            if ($product['image'] && is_file(DIR_IMAGE . $product['image']) && !empty($product['image'])) {
                ...
                foreach ($this->model_catalog_product->getProductImages($product['product_id']) as $image) {
                    $data['picture'][] = HTTPS_SERVER.'image/'.$image['image'];
                }
            }
            $attribute_groups = $this->model_catalog_product->getProductAttributes($product['product_id']);
            if ($attribute_groups) {
    ...
    private function hotlineTemplate($products, $setting=array()) {
        ...
        foreach ($products as $product) {
            ...
            $data['available'] = ($product['stock_status_id'] == $setting['in_stock_id']);
            ...
            $data['currencyId'] = $this->config->get('rozetka_currency');
            $data['categoryId'] = $product['hotline_category_id'];
            $name = $this->clearName($product['name']);
            $data['name'] = $name;
            if(!empty($product['manufacturer'])) {
               $data['vendor'] = $product['manufacturer']; 
    ...
    private function clearName($name) {
        ...
        $name = preg_replace('/&(?!#?[a-z0-9]+;)/', '&amp;', $fix_name);
        return $name;
    }
