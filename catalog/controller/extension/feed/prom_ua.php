...
class ControllerExtensionFeedPromUa extends Controller
{
    private $shop = array();
    ...
    public function index()
    {
        if ($this->config->get('rozetka_status')) {
            ...
            foreach($parent_category_ids as $parent_id) {
                if(!in_array($parent_id, $allowed_categories)) {

                     $category_parent_id = $this->model_export_rozetka->getCategory($parent_id);                     
            ...
            // parent level 2
            foreach($parent_category_ids2 as $parent_id) {
                if(!in_array($parent_id, $allowed_categories)) {
            ...
            // parent level 3
            foreach($parent_category_ids3 as $parent_id) {
                if(!in_array($parent_id, $allowed_categories)) {
...
//				$data['cbid'] = 15;
                ...
                if (!is_null($product['special_price'])) {
                    $data['price'] = number_format($this->currency->convert($this->tax->calculate($product['special_price'], $product['tax_class_id']), $shop_currency, $offers_currency), $decimal_place, '.', '');
...
//				$data['local_delivery_cost'] = 100;
				        $fix_name = str_replace('"', '\"', $product['name']);
				        $fix_name_full=preg_replace('/&(?!#?[a-z0-9]+;)/', '&amp;', $fix_name);

				        $fix_h1 = str_replace('"', '\"', $product['meta_h1']);
				        $fix_h1_full=preg_replace('/&(?!#?[a-z0-9]+;)/', '&amp;', $fix_h1);

                $data['name'] = $fix_h1_full;
                if(!empty($product['manufacturer'])) {
                   $data['vendor'] = $product['manufacturer']; 
                }
//				$data['vendorCode'] = $product['model'];
                $data['model'] = $fix_name_full;
...
//				$data['barcode'] = $product['sku'];
                if ($product['image'] && is_file(DIR_IMAGE . $product['image']) && !empty($product['image'])) {
                    ...
                    foreach ($this->model_catalog_product->getProductImages($product['product_id']) as $image) {
                        //$data['picture'][] = $this->model_tool_image->resize($image['image'], 600, 500);
   ...
   private function getYml($case, $number = false)
    {
        ...
        if($case == 'header' || $case == 'all') {
            ...
            if($case == 'header') {
                $filename = DIR_ROOT.'xml_export/marge_prom/header.xml';
        ...
        if($case == 'products' || $case == 'all') {
            ...
            if($case == 'products') {
                $filename = DIR_ROOT.'xml_export/marge_prom/product_'.$number.'.xml';
                if(file_put_contents($filename, $yml)) {
                  return "Файл $filename успешно сохранен";
                }
            }
        ...
        if($case == 'footer' || $case == 'all') {
            ...
            if($case == 'footer') {
                $filename = DIR_ROOT.'xml_export/marge_prom/zfooter.xml';
                if(file_put_contents($filename, $yml)) {
                  return "Файл $filename успешно сохранен";
                }
            } else {
    ...
    private function array2Param($params)
    {
        ...
        foreach ($params as $param) {
            $attrs = ['name="' . $this->prepareField($param['name']) . '"'];
            if (isset($param['unit'])) {
                $attrs[] = 'unit="' . $this->prepareField($param['unit']) . '"';
            }
            $retval .= '<param '.implode($attrs).'>' . $this->prepareField($param['value']) . '</param>' . $this->eol;
        }
    ...
    private function prepareField($field, $html = false)
    {
        ...
        if ($html) $field = preg_replace('/<p>\s*((\&nbsp;)|(<br\s*\/?>))?\s*<\/p>/', '', html_entity_decode($field));
        return trim($field);
    }
