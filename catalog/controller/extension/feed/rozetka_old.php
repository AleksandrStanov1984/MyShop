...
class ControllerExtensionFeedRozetka extends Controller
{
    ...
    public function index()
    {
        if ($this->config->get('rozetka_status')) {
            ...
            $imageFolder = realpath(DIR_IMAGE . "/../xml/images");
            ...
            // Категории
            $categories = $this->model_export_rozetka->getCategory();
            ...
            //$products = array_chunk($this->model_export_rozetka->getProduct($allowed_categories, $out_of_stock_id, $vendor_required), 50)[10]; //Выборка из середины
...
//				$data['cbid'] = 15;
                ...
                $pK = 0;
                $sK = 0;
                if (!is_null($product['price_special_rozetka'])) {
                    if ((int)$this->config->get('rozetka_percent_markup') > 0) {
                        $pK = 1 + ($this->config->get('rozetka_percent_markup') / 100);
                    ...
                    if ((int)$this->config->get('rozetka_percent_special_markup') > 0) {
                        $sK = 1 + ($this->config->get('rozetka_percent_special_markup') / 100);
                ...
                if (is_dir($productImageFolder)) {
                    ...
                    foreach ($files as $file) {
                        if (in_array($ext = substr($file, -3), $imageAvailableExt)) {
                            ...
                            $file = explode("/xml/images", $file);
                            array_shift($file);
                            $file = HTTPS_SERVER."xml/images".implode("/xml/images", $file);
                            $f($images, $file);
                            	//$this->model_tool_image->resize(substr($file, mb_strlen(DIR_IMAGE)), 600, 500));
                        }
                    }
                    $data['picture'] = $images;
                } else {
