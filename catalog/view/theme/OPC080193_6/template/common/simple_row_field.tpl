<div class="form-group <?php if ($required) { ?>required<?php } ?> row-<?php echo $id ?>">
    <!--<label class="control-label <?php echo $page == 'checkout' ? 'col-sm-4' : 'col-sm-2' ?> hidden" for="<?php echo $id ?>"><?php echo $label ?></label>-->
    <div class="<?php echo $page == 'checkout' ? '' : 'col-sm-10' ?>">
      <?php if ($type == 'select' || $type == 'select2') { ?>
        <div class="simple-select-block">
        <select class="form-control form-control-simple" name="<?php echo $name ?>" id="<?php echo $id ?>" <?php echo $bootstrap ? 'data-theme="bootstrap"' : '' ?> <?php echo $type == 'select2' ? 'data-type="select2"' : '' ?> <?php echo $reload ? 'data-onchange="reloadAll"' : 'data-reload-payment-form="true"'?>>
      ...
      <?php } else { ?>
        <?php if($id == 'customer_email') { ?>
          <?php if(stristr($value, 'localhost') !== FALSE) $value = ''; ?>
          <input  class="form-control form-control-simple <?php echo $required ? 'required-form-control-simple' : ''; ?>" type="<?php echo $type ?>" name="<?php echo $name ?>" id="<?php echo $id ?>" value="<?php echo preg_replace('/^380/','',$value) ?>" placeholder="<?php echo $placeholder ?>" <?php echo $attrs ?> <?php echo $reload ? 'data-onchange="reloadAll"' : 'data-reload-payment-form="true"'?>>
        <?php } else { ?>
          <input  class="form-control form-control-simple <?php echo $required ? 'required-form-control-simple' : ''; ?>" type="<?php echo $type ?>" name="<?php echo $name ?>" id="<?php echo $id ?>" value="<?php echo preg_replace('/^380/','',$value) ?>" placeholder="<?php echo $placeholder ?>" <?php echo $attrs ?> <?php echo $reload ? 'data-onchange="reloadAll"' : 'data-reload-payment-form="true"'?>>
        <?php } ?>
      <?php } ?>
